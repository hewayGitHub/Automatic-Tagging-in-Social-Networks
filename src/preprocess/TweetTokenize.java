...
public class TweetTokenize {
    ...
    private static List<String> addAllnonempty(List<String> master, List<String> smaller, boolean isGood){
    	if(isGood) { //need to make sure it is a valid English word and not a stop word
    		for (String s : smaller){
                String strim = s.trim().toLowerCase();
                if (strim.length() > 2 && isValidWord(strim) && !isStopWord(strim))
                    master.add(strim);
            }
    	} else { //only hashtags been preserved
    		for (String s : smaller){
                String strim = s.trim();
                if (strim.length() > 0 && strim.startsWith("#"))
                    master.add(strim);
            }
    	}
    ...
    static String[] stopWordStrs = {"rt", "a", "able", "about", "above", "according", "accordingly", "across", "actually", 
    	"after", "afterwards", "again", "against", "all", "allow", "allows", "almost", "alone", "along", 
    	"already", "also", "although", "always", "am", "among", "amongst", "an", "and", "another", "any", 
    	"anybody", "anyhow", "anyone", "anything", "anyway", "anyways", "anywhere", "apart", "appear", 
    	"appreciate", "appropriate", "are", "around", "as", "aside", "ask", "asking", "associated", 
    	...
    	"besides", "best", "better", "between", "beyond", "both", "brief", "but", "by", "c", "came", 
    	"can", "cannot", "cant", "cause", "causes", "certain", "certainly", "changes", "clearly", 
    	"co", "com", "come", "comes", "concerning", "consequently", "consider", "considering", 
    	"contain", "containing", "contains", "corresponding", "could", "course", "currently", "d", 
    	...
    	"insofar", "instead", "into", "inward", "is", "it", "its", "itself", "j", "just", "k", "keep", 
    	"keeps", "kept", "know", "knows", "known", "l", "last", "lately", "later", "latter", "latterly", 
    	...
    	"selves", "sensible", "sent", "serious", "seriously", "seven", "several", "shall", "she", "should", "since", 
    	...
    	"which", "while", "whither", "who", "whoever", "whole", "whom", "whose", "why", "will", "willing", "wish", 
    	"with", "within", "without", "wonder", "would", "would", "x", "y", "yes", "yet", "you", "your", "yours", 
    	"yourself", "yourselves", "z", "zero"};
    static Set<String> stopWords = new HashSet<String>();
    static {
    	for(String sw: stopWordStrs) {
    		stopWords.add(sw);
    	}
    }
    ...
    public static boolean isValidWord(String token) { 
        for (int i = 0; i < token.length(); i++) {
        	if(token.charAt(i) == '-' && i != token.length() - 1) continue;
