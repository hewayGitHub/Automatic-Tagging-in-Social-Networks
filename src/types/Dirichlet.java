...
public class Dirichlet {
	 ...
	 * 
	 * @param m
	 *            The magnitude of the Dirichlet: sum_i alpha_i
	 * @param p
	 *            A probability distribution: p_i = alpha_i / m
	 */
	...
	public static String distributionToString(double magnitude,
			double[] distribution) {
	 ...
	 * 
	 * @param n
	 *            The <i>expected</i> total number of counts in the returned
	 ...
	 * 
	 * @param z
	 *            Note that Stirling's approximation is increasingly unstable as
	 *            <code>z</code> approaches 0. If <code>z</code> is less than 2,
	 *            we shift it up, calculate the approximation, and then shift
	...
	public static double logGammaStirling(double z) {
		...
		double result = HALF_LOG_TWO_PI + (z - 0.5) * Math.log(z) - z + 1
				/ (12 * z) - 1 / (360 * z * z * z) + 1
				/ (1260 * z * z * z * z * z);
	...
	public static double logGammaNemes(double z) {
		double result = HALF_LOG_TWO_PI - (Math.log(z) / 2) + z
				* (Math.log(z + (1 / (12 * z - (1 / (10 * z))))) - 1);
		return result;
	}
	 ...
	 *            An array of frequencies. If the matrix X represents
	 *            observations such that x<sub>dt</sub> is how many times word t
	 *            occurs in document d, <code>countHistogram[3]</code> is the
	 *            total number of cells <i>in any column</i> that equal 3.
	 * @param observationLengths
	 *            A histogram of sample lengths, for example
	 *            <code>observationLengths[20]</code> could be the number of
	 *            documents that are exactly 20 tokens long.
	 * @param numDimensions
	...
	public static double learnSymmetricConcentration(int[] countHistogram,
			int[] observationLengths, int numDimensions, double currentValue) {
		...
		for (int iteration = 1; iteration <= 200; iteration++) {
			...
			for (denseIndex = 0; denseIndex < denseIndexSize; denseIndex++) {
				...
				if (length - previousLength > 20) {
					...
					currentDigamma = digamma(currentValue + length)
							- cachedDigamma;
				} else {
					// Otherwise iterate up. This looks slightly different
					// from the previous version (no -1) because we're indexing
					// differently.
	...
	public static void testSymmetricConcentration(int numDimensions,
			int numObservations, int observationMeanLength) {
		...
		for (int exponent = -5; exponent < 4; exponent++) {
			...
			Dirichlet prior = new Dirichlet(numDimensions, alpha
					/ numDimensions);

			int[] countHistogram = new int[1000000];
			int[] observationLengths = new int[1000000];

			Object[] observations = prior.drawObservations(numObservations,
					observationMeanLength);
			...
			double estimatedAlpha = learnSymmetricConcentration(countHistogram,
					observationLengths, numDimensions, 1.0);
	 ...
	 * 
	 * @param parameters
	 *            A reference to the current values of the parameters, which
	 *            will be updated in place
	 * @param observations
	 *            An array of count histograms. <code>observations[10][3]</code>
	 *            could be the number of documents that contain exactly 3 tokens
	 *            of word type 10.
	 * @param observationLengths
	 *            A histogram of sample lengths, for example
	 *            <code>observationLengths[20]</code> could be the number of
	...
	public static double learnParameters(double[] parameters,
			int[][] observations, int[] observationLengths) {
	 ...
	 *            documents that are exactly 20 tokens long.
	 * @param shape
	 *            Gamma prior E(X) = shape * scale, var(X) = shape *
	 *            scale<sup>2</sup>
	 * @param scale
	 * @param numIterations
	 *            200 to 1000 generally insures convergence, but 1-5 is often
	 *            enough to step in the right direction
	 * @returns The sum of the learned parameters.
	 */
	public static double learnParameters(double[] parameters,
			int[][] observations, int[] observationLengths, double shape,
			double scale, int numIterations) {
		int i, k;
		...
		for (int iteration = 0; iteration < numIterations; iteration++) {
			...
			for (k = 0; k < parameters.length; k++) {
				...
				parameters[k] = (oldParametersK * parameters[k] + shape)
						/ denominator;
	...
	public long learnParametersWithHistogram(Object[] observations) {
		...
		// System.out.println("got mem: " + (System.currentTimeMillis() -
		// start));

		int[] lengthHistogram = new int[maxLength + 1];
		Arrays.fill(lengthHistogram, 0);
		// System.out.println("got lengths: " + (System.currentTimeMillis() -
		// start));
		...
		for (int iteration = 0; iteration < 1000; iteration++) {
			...
			for (k = 0; k < partition.length; k++) {
				...
				if (!(newParameters[k] > 0.0)) {
					System.out.println("length of empty array: "
							+ (new int[0]).length);
			 ...
			 * //System.out.println(distributionToString(parametersSum,
			 * newParameters)); //toFile("../newsgroups/direct/iteration" +
	...
	public long learnParametersWithDigamma(int[][] binCounts,
		...
		for (int iteration = 0; iteration < 1000; iteration++) {
			...
			for (k = 0; k < partition.length; k++) {
				...
				if (newParameters[k] <= 0) {
					System.out.println(newParameters[k] + "\t" + alphaK + "\t"
							+ denominator);
				}
			...
			for (k = 0; k < partition.length; k++) {
				...
				/*
				 * if (k < 20) {
				 * System.out.println(partition[k]+" = "+newParameters
				 * [k]+" / "+magnitude); }
				 */
			}
	...
	public long learnParametersWithMoments(Object[] observations) {
		...
		for (i = 0; i < observations.length; i++) {
			...
			for (bin = 0; bin < partition.length; bin++) {
				partition[bin] += (double) observation[bin]
						/ observationLengths[i];
			}
		}
		...
		for (i = 0; i < observations.length; i++) {
			...
			for (bin = 0; bin < partition.length; bin++) {
				difference = ((double) observation[bin] / observationLengths[i])
						- partition[bin];
				variances[bin] += difference * difference; // avoiding
															// Math.pow...
			}
		}
		...
		// log \sum_k \alpha_k = 1/(K-1) \sum_k log[ ( E[p_k](1 - E[p_k]) /
		// var[p_k] ) - 1 ]
		...
		for (bin = 0; bin < partition.length; bin++) {
			...
			sum += Math
					.log((partition[bin] * (1 - partition[bin]) / variances[bin]) - 1);
		}
		...
		for (int iteration = 0; iteration < 1000; iteration++) {
			...
			for (bin = 0; bin < partition.length; bin++) {
				if (binSums[bin] == 0.0) {
					...
					newParameters[bin] = (partition[bin] * magnitude
							* binSums[bin] / observationSum);
				}
			...
			/*
			 * if (iteration % 50 == 0) { System.out.println(iteration + ": " +
			 * magnitude); }
			 */
		}
	...
	public double absoluteDifference(Dirichlet other) {
		if (partition.length != other.partition.length) {
			throw new IllegalArgumentException(
					"dirichlets must have the same dimension to be compared");
		}
	...
	public void checkBreakeven(double x) {
		...
		for (int n = 1; n < 100; n++) {
			...
			System.out.println(n + "\tdirect: " + clock1 + "\tindirect: "
					+ clock2 + " (" + (clock1 - clock2) + ")");
			System.out.println("  " + (digamma(x + n) - digammaX) + " "
					+ digammaDifference(x, n));
	...
	public static String compare(double sum, int k, int n, int w) {
		...
		// distributionToString(dirichlet.magnitude,
		// dirichlet.partition));
	 ...
	 * from this Dirichlet?
	 */
	public static double dirichletMultinomialLikelihoodRatio(
			TIntIntHashMap countsX, TIntIntHashMap countsY, double alpha,
			double alphaSum) {
		...
		logLikelihood += logGamma(alphaSum + totalX)
				+ logGamma(alphaSum + totalY) - logGamma(alphaSum)
	...
	public static double dirichletMultinomialLikelihoodRatio(int[] countsX,
			int[] countsY, double alpha, double alphaSum) {
		...
		if (countsX.length != countsY.length) {
			throw new IllegalArgumentException(
					"both arrays must contain the same number of dimensions");
		}
	...
	public double dirichletMultinomialLikelihoodRatio(int[] countsX,
		...
		if (countsX.length != countsY.length
				|| countsX.length != partition.length) {
			throw new IllegalArgumentException(
		...
		logLikelihood += logGamma(magnitude + totalX)
				+ logGamma(magnitude + totalY) - logGamma(magnitude)
	 ...
	 * restaurant process.
	 */
	public static double ewensLikelihoodRatio(int[] countsX, int[] countsY,
			double lambda) {
		...
		for (int j = 1; j <= total; j++) {
			if (countHistogramX[j] == 0 && countHistogramY[j] == 0
			...
			logLikelihood += logGamma(countHistogramX[j] + 1)
					+ logGamma(countHistogramY[j] + 1)
					- logGamma(countHistogramBoth[j] + 1);

		}

		logLikelihood += logGamma(total + 1) - logGamma(totalX + 1)
				- logGamma(totalY + 1);
				...
				for (int k = 0; k < 5; k++) {
					...
					for (int l = 0; l < 5; l++) {
						...
						for (int m = 0; m < 10; m++) {
							...
							out.println(compare(dimensions, dimensions,
									documents, meanSize));
						}
						out.flush();
						meanSize *= 2;
	...
	public static void main(String[] args) {
		...
		/*
		 * 
		 * Dirichlet prior = new Dirichlet(100, 1.0); double[] distribution;
		 * int[] x, y;
		 * 
		 * for (int i=0; i<50; i++) {
		 * 
		 * Dirichlet nonSymmetric = new Dirichlet(100,
		 * prior.nextDistribution());
		 * 
		 * // Two observations from same multinomial distribution =
		 * nonSymmetric.nextDistribution(); x =
		 * nonSymmetric.drawObservation(100, distribution); y =
		 * nonSymmetric.drawObservation(100, distribution);
		 * 
		 * System.out.print(nonSymmetric.dirichletMultinomialLikelihoodRatio(x,
		 * y) + "\t"); System.out.print(ewensLikelihoodRatio(x, y, 1) + "\t");
		 * 
		 * // Two observations from different multinomials
		 * 
		 * x = nonSymmetric.drawObservation(100); y =
		 * nonSymmetric.drawObservation(100);
		 * 
		 * System.out.print(ewensLikelihoodRatio(x, y, 0.1) + "\t");
		 * System.out.println
		 * (nonSymmetric.dirichletMultinomialLikelihoodRatio(x, y)); }
		 */
	}
	...
	public void print() {
		...
		for (int j = 0; j < partition.length; j++)
			System.out.println(dict != null ? dict.lookupObject(j).toString()
					: j + "=" + magnitude * partition[j]);
	}
	...
	protected double[] randomRawMultinomial(Randoms r) {
		...
		for (int i = 0; i < this.partition.length; i++) {
			...
			// System.out.println (dict.lookupSymbol(j).toString() + "=" +
			// alphas[j]);
	...
	// {
	// return new Multinomial (randomRawMultinomial(r), dict, partition.length,
	// false, false);
	// }
	//
