...
public class RankedList {
	...
	/**
	 * values save (id, weight)
	 * After calling sortListDesc, indices is saved ids desc on weight.
	 */
	...
	public double KendallTauCoefficient(RankedList list) {
		double coeff = 0;
		...
		for (int i = 0; i < keys.length; i++) {
			if (values.containsKey(keys[i])) {
				group2.adjustOrPutValue(list.values.get(keys[i]), 1, 1);
			}
		}
		int ties1 = 0, ties2 = 0;
