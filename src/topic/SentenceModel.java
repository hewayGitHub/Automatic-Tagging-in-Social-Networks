...
import types.SentenceCorpus;
import types.Dirichlet;
import types.SentenceDocument;
...
public class SentenceModel implements Serializable{
	...
	public double betaBackgroundSum;
	public double gamma;
	...
	public double[] phiBackground_train;
	...
	public void InitializeParameters(SentenceCorpus corpus, int numTopics) {
		this.numTopics = numTopics;
		numDocs = corpus.numDocs = SentenceCorpus.docNameAlphabet.size();
		numSentences = corpus.numSentences;
		numUniqueWords = corpus.numUniqueWords = SentenceCorpus.vocabulary.size();
		numUniqueCitations = corpus.numUniqueCitations = SentenceCorpus.citationAlphabet.size();
		...
		phi_train = new double[numTopics][numUniqueWords]; // topic-word distribution
		phiBackground_train = new double[numUniqueWords];
		psi_train = new double[numTopics][numUniqueCitations]; //topic-citation distribution
	}

	public void InitializeAssignments(SentenceCorpus corpus, LabelAlphabet topicAlphabet) {
		// initilize word, and citation factors
		this.random = new Randoms();
		for (int doc_index = 0; doc_index < numDocs; doc_index++) {
			SentenceDocument doc = corpus.getDoc(doc_index);
	...
	public void sampleOneDocument(SentenceCorpus corpus, SentenceDocument doc) {
	...
	public void sampleCorpus(SentenceCorpus corpus, int iterations){
	...
	public void estimateParameters(SentenceCorpus corpus) {
		// estimate parameters after every k iterations after burn-in
		numSamples++;

		for (int m = 0; m < numDocs; m++) {
			SentenceDocument doc = corpus.getDoc(m);
	...
	public void estimateParameters_single(SentenceCorpus corpus) {
		// estimate parameters after every k iterations after burn-in
		numSamples++;
		for (int m = 0; m < numDocs; m++) {
			SentenceDocument doc = corpus.getDoc(m);
	 ...
	 */
	public ArrayList<TreeSet<IDSorter>> getSortedWords(SentenceCorpus corpus, int topN) {
	...
	public ArrayList<TreeSet<IDSorter>> getSortedCitations(SentenceCorpus corpus, int topN) {
	...
	public double testPerplexity(int numSamples, SentenceCorpus corpus) {
		double logSum = 0;
		int sampleSize = 0;
	...
	public double modelLogLikelihood(SentenceCorpus corpus) {
	...
	public ArrayList<TreeSet<IDSorter>> recommendWord(int topN) {
		...
		for(int m = 0; m < numDocs; m++) {
			recSortedWords.add(recommendWord(m, topN));
		}
	...
	public ArrayList<TreeSet<IDSorter>> recommendCitation(int topN) {
		...
		for(int m = 0; m < numDocs; m++) {
			recSortedCitation.add(recommendCitation(m, topN));
		}
		
		return recSortedCitation;
	}
	
	public Alphabet vocabulary = SentenceCorpus.vocabulary;
	public Alphabet citationAlphabet = SentenceCorpus.citationAlphabet;
	public Alphabet docNameAlphabet = SentenceCorpus.docNameAlphabet; // save doc name
